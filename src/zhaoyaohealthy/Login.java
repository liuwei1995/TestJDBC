...
//				JSON.parseObject(jsonObject.toString(), Drug.class);
		...
		try {
			Drug newInstance = Drug.class.newInstance();
			Field[] fields = Drug.class.getDeclaredFields();
			for (Field field : fields) {
				field.setAccessible(true);
				if (field.getName().equals("payment")) {
//					field.set(newInstance, Float.parseFloat("2400.00"));
					field.setFloat(newInstance, Float.parseFloat("2400.00"));
				}
			}
			System.out.println(newInstance.toString());
			float payment = newInstance.getPayment();
