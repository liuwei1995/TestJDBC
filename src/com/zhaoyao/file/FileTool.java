...
public class FileTool {
	...
	public static String getLocalFileEncode(String localFile) {
		return getLocalFileEncode(new File(localFile));
	}
    /**
     * 获取本地文件的编码格式
     * 
    ...
    public static void main(String[] args) {
		...
		try {
			stringValue = new String("你好".getBytes(), "gbk");
		} catch (UnsupportedEncodingException e) {
			e.printStackTrace();
		}
    	String stringEncode = getStringEncode(stringValue);
    	System.out.println(stringEncode);
    	try {
			String drugCnName = new String(stringValue.getBytes(stringEncode), "utf-8");
			System.out.println(drugCnName);
		} catch (UnsupportedEncodingException e) {
